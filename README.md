...
## admin.html 수동 사운드 검색 방식 변경

- 기존 드롭다운 선택 방식 대신 검색 입력으로 변경했습니다.
- `public/sounds/` 폴더에 있는 파일명을 `/api/sounds`로 불러옵니다.
- admin.html에서 `9900`, `윈도우`, `대박`처럼 검색하면 파일명에 해당 단어가 들어간 사운드가 표시됩니다.
- 검색 결과를 클릭하면 해당 후원 Alert에서 수동 사운드로 우선 재생됩니다.
- 비워두면 자동 규칙이 적용됩니다: 프리셋 사운드 → 금액구간 사운드 → 기본 사운드.
