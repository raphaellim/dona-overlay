...
- optionAlert를 creatorBox 밖으로 이동하여 크리에이터박스 OFF 상태에서도 ALERT가 표시됩니다.
- settings.notice가 문자열인 기존 구조와 notice 객체 구조를 모두 지원합니다.
- 공지 제목, 내용, 줄바꿈, 줄별 색상이 즉시 렌더링되도록 보정했습니다.
- 공지는 매 refresh마다 렌더링되어 control.html 저장 후 overlay에 더 안정적으로 반영됩니다.
