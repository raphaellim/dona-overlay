...
## 사운드 설정 / 수동 사운드 / 공지 색상 저장 수정

### control.html
- 금액별/옵션별 사운드 파일명 설정 카드 추가
- 사운드 사용 ON/OFF 추가
- 공지 1~5줄 컬러 저장값이 다시 기본값으로 돌아가던 문제 수정

### admin.html
- 입력 화면에 수동 사운드 선택 추가
- Alert 표시 사운드 제목 입력 추가
- 수동 사운드를 선택하면 자동 규칙보다 우선 재생

### overlay.html
- control.html의 사운드 설정값을 읽어 파일명을 적용
- manualSoundKey가 있으면 수동 사운드 우선 재생
- manualSoundTitle이 있으면 Alert에 뱃지처럼 표시

### DB
기존 DB에는 `supabase_sound_manual_update.sql`을 1회 실행하세요.
