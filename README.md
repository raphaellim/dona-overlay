...
control.html 오버레이 노출 설정에 `크리에이터 후원 표시` 체크박스를 추가했습니다.

- ON: 기존처럼 `크리에이터 - 후원(금액)` + 옵션 표시
- OFF: 후원금액 숨김, 크리에이터 이름 + 옵션만 표시
- `크리에이터박스` 자체를 OFF 하면 크리에이터 박스 전체가 숨겨집니다.
- ALERT 동작과는 독립입니다.


## 최종 반영 오류 수정

1. 새 창/서버 웨이크업 후 과거 후원 ALERT 재출력 방지.
2. 모바일 ALERT `후원자 → 크리에이터` 한 줄 유지 보정.
3. `빵떠기`, `또영이` 하드코딩 기본 크리에이터 제거.
4. 공지 제목/내용/줄바꿈/줄별 색상 렌더링 보정.
5. 크리에이터박스 OFF/후원표시 OFF와 ALERT 동작 분리 유지.


## 수동 사운드 대기열 claim/lease 수정

문제:
- 오버레이 창 2개를 열면 둘 중 하나가 대기열을 소비해서 다른 창에서 안 나옴.
- 재생 중 창이 멈추면 남은 횟수가 전송됨 상태로 남을 수 있음.

수정:
- `/api/sound-events/:id/claim` 추가.
- 실제 player 오버레이만 대기열을 claim.
- claim은 90초 lease로 동작해서 창이 죽으면 다른 player가 이어받을 수 있음.
- PC 모니터링 창은 `monitor=1`을 붙여 대기열을 훔쳐가지 않게 함.

권장 URL:
- 프리즘 실제 방송용: `/overlay.html?station=duugi&token=토큰&player=1`
- PC 모니터링용: `/overlay.html?station=duugi&token=토큰&monitor=1`


## 대기열 멈춤/재전송 + claim/lease

- 실제 방송용 오버레이만 대기열을 소비합니다.
  - 프리즘 방송용: `/overlay.html?station=duugi&token=토큰&player=1`
  - PC 모니터링용: `/overlay.html?station=duugi&token=토큰&monitor=1`
- 대기열 항목은 claim/lease 방식으로 잡습니다.
- 멈춤 버튼:
  - 현재 재생 중인 회차는 끝까지 진행
  - 다음 횟수부터 queued로 멈춤
- 재전송 버튼:
  - queued 상태의 남은 횟수부터 다시 pending으로 전송
- 재생 중 창이 죽으면 claim lease 만료 후 다른 player가 이어받을 수 있습니다.
